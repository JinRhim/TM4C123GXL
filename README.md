...
1. Exceptions - handled by NVIC (Nested Vectored Interrupt Controller)
2. Interrupt (IRQ)
	1. exception signaled by hardware --> NVIC 
	2. generated by software request --> NVIC
3. Peripherals - use interrupt to communicate with Processor 
4. Each Exception - 32-bit vector to memory location ==> Interrupt Service Routine (ISR) location
5. Processors handle Exception using 
	1. Interrupt Service Routines (ISR) - handle IRQx
	2. Fault Handler 
		1. Hard Fault 
